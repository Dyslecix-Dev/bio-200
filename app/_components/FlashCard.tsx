...
export default function FlashCardComponent({ tableKey, tableValue }: { tableKey: string; tableValue: string }) {
  return (
    <main className="min-h-screen overflow-hidden bg-zinc-950">
      <Navbar />
      <CardGrid tableKey={tableKey} tableValue={tableValue} />
      <Beams />
      <GradientGrid />
    </main>
  );
...
const CardGrid = ({ tableKey, tableValue }: { tableKey: string; tableValue: string }) => {
  ...
  useEffect(() => {
    const fetchFlashCards = async () => {
      try {
        ...
        const { data, error } = await supabase.from("flash_cards").select("*").eq(tableKey, tableValue);

        if (error) {
          setError(error.message);
          console.error("Error fetching flash cards:", error);
        } else if (data) {
          const transformedData = data.map(({ front_text, back_text, front_image, back_image, id, topic, grade, ...rest }) => ({
            id,
            topic,
            grade,
  ...
  return (
    <div className="relative cursor-pointer aspect-square w-full max-w-sm mx-auto" onClick={handleClick}>
      <motion.div className="relative w-full h-full" style={{ transformStyle: "preserve-3d" }} animate={{ rotateY: isFlipped ? 180 : 0 }} transition={{ duration: 0.6, ease: "easeInOut" }}>
        ...
        >
          <div className="flex items-center justify-center min-h-full">{backText && <h3 className="text-xl font-bold text-white text-center leading-tight">{backText}</h3>}</div>
        </div>
      </motion.div>
    </div>
  );
